(function () {
    ...
    var processRequest = function (request, response) {
        if (shouldInterceptFile(request, response)) {
            ...
            return { redirectUrl: "https://clients1.google.com/generate_204" };
        } else {
    ...
    var sendToXDM = function (request, response, file, video) {
        ...
        chrome.cookies.getAll({ "url": response.url }, function (cookies) {
            ...
            log(data);
            $.post(xdmHost + (video ? "/video" : "/download"), data).done(function () {
                log("data sent successfully");
            }).fail(function () {
                log("data sent failure");
            });
        });
    };
    ...
    var syncWithXDM = function () {
        $.getJSON(xdmHost + "/sync", function (data) {
    ...
    var initSelf = function () {
        ...
        setInterval(function () { syncWithXDM(); }, 1000);
